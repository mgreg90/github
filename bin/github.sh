...
# Get ruby version
PLAYWRIGHT_GITHUB_RUBY_VERSION=$(ruby -ryaml -e " puts (YAML.load_file(\"#{ENV['HOME']}/.playwright/plays/github/config.yml\")[:ruby_version] || '').gsub('.', '')")

# command to run proper ruby version
PLAYWRIGHT_RUBY_COMMAND="$HOME/.playwright/rubies/ruby$PLAYWRIGHT_GITHUB_RUBY_VERSION"
...
GEM_HOME="$HOME/.playwright/plays/github/gems"
# Set gem path to use gems from local folder, as well as normal paths
GEM_PATH="$GEM_HOME:$GEM_PATH"

# Run play
$PLAYWRIGHT_RUBY_COMMAND -W0 $HOME/.playwright/plays/github/github "$@"
